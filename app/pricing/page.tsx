    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 overflow-x-hidden">
      ...
      <header className="relative z-10 border-b border-zinc-800/40">
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <Link href="/login" className="flex items-center gap-1.5 text-xs font-medium text-zinc-400 hover:text-zinc-100 transition-colors">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 pt-20 pb-24 space-y-16">
        ...
        <div className="text-center space-y-4 max-w-xl mx-auto">
          ...
          {/* Billing toggle */}
          <div className="inline-flex items-center gap-1 bg-zinc-900 border border-zinc-800 rounded-xl p-1 mt-2">
        ...
        <div className="grid sm:grid-cols-3 gap-5 items-start">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-6 space-y-5">
            ...
            <Link
              href="/login"
              className="block text-center w-full py-2.5 rounded-xl border border-zinc-700 text-xs font-semibold text-zinc-300 hover:border-zinc-500 transition-all"
            >
              Get started free
            </Link>
          </div>
          ...
          {(Object.entries(PLANS) as [PlanKey, typeof PLANS[PlanKey]][]).map(([key, plan], i) => (
            ...
            >
              ...
              <div className="space-y-1">
                <p className="text-sm font-semibold text-zinc-200">{plan.name}</p>
              ...
              <ul className="space-y-2">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-xs text-zinc-300">
                    <Check className={`w-3.5 h-3.5 mt-0.5 flex-shrink-0 ${i === 0 ? 'text-emerald-400' : 'text-zinc-400'}`} />
                    {f}
                  </li>
                ))}
              </ul>
        ...
        {/* FAQ row */}
        <div className="border-t border-zinc-800/50 pt-12 grid sm:grid-cols-3 gap-8">
          {[
