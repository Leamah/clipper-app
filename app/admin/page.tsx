...
import { Shield, ArrowLeft, Loader2, Save, Infinity, MessageSquare, Bug, Lightbulb, MessageCircle } from 'lucide-react'
...
const FEEDBACK_TYPE_CONFIG = {
  bug:   { label: 'Bug',   icon: <Bug          className="w-3 h-3" />, color: 'bg-red-500/20 text-red-300'    },
  idea:  { label: 'Idea',  icon: <Lightbulb    className="w-3 h-3" />, color: 'bg-amber-500/20 text-amber-300' },
  other: { label: 'Other', icon: <MessageCircle className="w-3 h-3" />, color: 'bg-zinc-700 text-zinc-400'     },
}
...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (tab !== 'feedback' || fbLoaded) return
    setFbLoading(true)
    fetch('/api/admin/feedback')
      .then((r) => r.json())
      .then((d) => { setFeedback(d.feedback ?? []); setFbLoaded(true) })
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-8">
        ...
        <div className="flex gap-1 border-b border-zinc-800">
          {([
            ...
            { key: 'feedback', label: 'Feedback', icon: <MessageSquare className="w-3.5 h-3.5" /> },
          ] as const).map((t) => (
            <button
              key={t.key}
        ...
        {tab === 'feedback' && (
          fbLoading ? (
            ...
            <div className="text-center py-16 text-zinc-500 text-sm">No feedback yet</div>
          ) : (
            <div className="space-y-3">
              {feedback.map((fb) => {
                const cfg = FEEDBACK_TYPE_CONFIG[fb.type] ?? FEEDBACK_TYPE_CONFIG.other
                return (
                  <div key={fb.id} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${cfg.color}`}>
                        {cfg.icon}{cfg.label}
                      </span>
                      {fb.page && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-zinc-800 text-zinc-400 font-mono">{fb.page}</span>
                      )}
                      <span className="ml-auto text-xs text-zinc-600">
                        {new Intl.DateTimeFormat('en', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }).format(new Date(fb.created_at))}
                      </span>
                    </div>
                    <p className="text-sm text-zinc-200 leading-relaxed whitespace-pre-wrap">{fb.message}</p>
                    {fb.email && <p className="text-xs text-zinc-500">{fb.email}</p>}
                  </div>
                )
              })}
            </div>
